...
public class TextDecryption {
    public static void main(String[] args) {
        String propFilePath = "src\\main\\resources\\sdxl_prop.txt";
        String templateFilePath = "src\\main\\resources\\sdxl_template.txt";
        String outputFilePath = "src\\main\\resources\\sdxl.txt";
    ...
    private static String replacePlaceholder(String line, List<String> propLines) {
        ...
        while (matcher.find()) {
            ...
            if (function.equals("natureOrder")) {
                ...
                List<String> sortedLines = new ArrayList<>(propLines);
                Collections.sort(sortedLines, Comparator.comparing(TextDecryption::getAlphaChars));
                replacement = sortedLines.get(index);
            } else if (function.equals("charOrderDESC")) {
                List<String> sortedLines = new ArrayList<>(propLines);
                Collections.sort(sortedLines, Comparator.comparing(TextDecryption::getAlphaChars).reversed());
                replacement = sortedLines.get(index);
            }
